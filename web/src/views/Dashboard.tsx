  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        {/* Card 9: Managed Session Refresh */}
        <div className="bg-surface rounded-2xl p-5 border border-border shadow-sm hover:shadow-md transition-all duration-300 animate-slide-up" style={{ animationDelay: '0.45s' }}>
          <div className="flex items-center justify-between mb-4">
            <div className="text-[14px] font-medium text-textSecondary">会话刷新</div>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${status?.managed_refresh_running ? 'bg-cyan-50 text-cyan-600 dark:bg-cyan-900/30 dark:text-cyan-400' : 'bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400'}`}>
          ...
          <div className={`text-[24px] font-bold tracking-tight ${status?.managed_refresh_running ? 'text-success' : 'text-danger'}`}>
            {status?.managed_refresh_running ? 'Running' : 'Stopped'}
          </div>
          <div className="text-[12px] text-textMuted mt-1">
            每 {status?.managed_refresh_interval_seconds || 300}s 检查，提前 {status?.managed_refresh_lead_seconds || 600}s 刷新
          </div>
        </div>
